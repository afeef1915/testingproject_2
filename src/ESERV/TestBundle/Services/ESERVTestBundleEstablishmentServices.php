...
class ESERVTestBundleEstablishmentServices extends Controller {
    ...
    //list centre cases
    public function ListEstablishments($type = 'doctrine', $alias = 'p', $query_only = false) {
        $qb = $this->em->createQueryBuilder();

        $result = $qb->select($alias)
                        ->from('ESERVTestBundle:GtcwVEstablishment', $alias);

        if (!$result) {
            throw $this->createNotFoundException(
                    'No Establishments found-- Function "ListEstablishments"'
            );
        } else {
            if (!$query_only) {
