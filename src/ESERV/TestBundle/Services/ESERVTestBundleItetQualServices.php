...
class ESERVTestBundleItetQualServices extends Controller {
    ...
    //list centre cases
    public function ListItetQual($contact_id, $type = 'doctrine', $alias = 'p', $query_only = false) {
        $qb = $this->em->createQueryBuilder();

        $result = $qb->select($alias)
                        ->from('ESERVTestBundle:GtcwVItetQual', $alias)
        ...
        if (!$result) {
            throw $this->createNotFoundException(
                    'No ITET details found-- Function "ListItetQual"'
            );
        } else {
            if (!$query_only) {
