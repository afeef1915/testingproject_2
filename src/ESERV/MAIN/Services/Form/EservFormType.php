...
class EservFormType extends \Symfony\Component\Form\AbstractTypeExtension {
    ...
    public function __construct(PropertyAccessorInterface $propertyAccessor = null, Container $C = null) {
    ...
    public function buildView(FormView $view, FormInterface $form, array $options) {
        ...
        $data_eservvalidation = array();
        $data_eserv_autocomplete_options = array();
        $data_eserv_lettercase = array();

        if ($form->isRequired()) {
            $data_eservvalidation['required'] = 'required';
        }

        if (isset($options['eserv_extra_validation'])) {
            foreach ($options['eserv_extra_validation'] as $key => $value) {
                $data_eservvalidation[$key] = $value;
            }
        }
        $eservChoiceAttr = array();
        if (isset($options['eserv_choice_attr'])) {
            foreach ($options['eserv_choice_attr'] as $key => $value) {
                $eservChoiceAttr[$key] = $value;
            }
        }

        if (isset($options['eserv_autocomplete_options'])) {
            foreach ($options['eserv_autocomplete_options'] as $key => $value) {
                $data_eserv_autocomplete_options[$key] = $value;
            }
        }

        $extraParam = array();
        if (isset($options['extra_param'])) {
            $extraParam = $options['extra_param'];
        }

        /* SEE C:\xampp\htdocs\merlin\src\ESERV\MAIN\HelpersBundle\Resources\views\Form\eserv_layout.html.twig 
         * for html side of data_eservvalidation */
        $is_date = key_exists('date', $data_eservvalidation) ? $data_eservvalidation['date'] : false;
        $isDateTime = key_exists('date_time', $data_eservvalidation) ? $data_eservvalidation['date_time'] : false;
        $is_regx = key_exists('regexp', $data_eservvalidation) ? $data_eservvalidation['regexp'] : false;
        $is_lettercase = key_exists('lettercase', $data_eservvalidation) ? $data_eservvalidation['lettercase'] : false;

        $treeOptions = key_exists('tree_options', $options) ? $options['tree_options'] : array();
        ...
        $view->vars = array_replace($view->vars, array(
            ...
            'dataESERVValidation' => implode(',', array_unique(array_keys($data_eservvalidation))),
            'dataESERVACOptions' => $data_eserv_autocomplete_options,
            'eserv_choice_attr' => $eservChoiceAttr,
            'is_date' => $is_date,
            'is_date_time' => $isDateTime,
            'is_regx' => $is_regx,
            'is_lettercase' => $is_lettercase,
    ...
    public function configureOptions(OptionsResolver $resolver) {
        ...
        $resolver->setDefaults(array(
            ...
            'eserv_choice_attr' => array(),
            'extra_param' => array(),
            'translation_domain' => !is_null($this->c) ? $this->c->get('eserv_translation_services')->getFullEservDbTransDomain() : null,
        ));
