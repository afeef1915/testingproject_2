...
namespace ESERV\MAIN\FrameworkChanges;
 ...
 */
class MtlPdoSessionHandler implements \SessionHandlerInterface {
        ...
        try {
            ...
            if ($sessionRows) {
                if (is_resource($sessionRows[0][0])) {
                    $result = base64_decode(stream_get_contents($sessionRows[0][0]));
                } else {
                    $result = base64_decode($sessionRows[0][0]);
                }
            }
    ...
    public function write($sessionId, $data) {
        ...
        if (is_resource($data)) {
            $encoded = base64_encode(stream_get_contents($data));
        } else {
            $encoded = base64_encode($data);
        }
...
//            }
            ...
            try {
                ...
                $insertStmt->bindParam(':data', $encoded, \PDO::PARAM_STR, 50000);
