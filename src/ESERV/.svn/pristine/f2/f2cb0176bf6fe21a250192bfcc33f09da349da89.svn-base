...
class MediaBundleMediaStoreService extends Controller {
    ...
    public function show($id, $relationships = true, $type = 'array') {
        ...
        if ($relationships === true) {
            $result = $result->select('p', 'mt')
                    ->from('ESERVMAINMediaBundle:MediaStore', 'p')
    ...
    public function delete($id) {
        $result = $this->em->createQueryBuilder()
                        ->delete('ESERVMAINMediaBundle:MediaStore', 'p')
                        ->where('p.id = :id')
    ...
    {
        $em = $this->em;
        $ent = $em->getRepository('ESERVMAINMediaBundle:MediaStore')->findOneBy(
                    array(
    ...
    public function updateEntityNameAndId($id, $entityName, $entityId)
    {
        if($id && $entityName && $entityId){
            $media_type = $this->em->getRepository('ESERV\MAIN\MediaBundle\Entity\MediaStore')->find($id);
    ...
    {
        $em = $this->em;
        $media_store = $em->getRepository('ESERVMAINMediaBundle:MediaStore')->findBy(
                    array(
                        'entityName' => $entity_name,
                        'entityId' => $entity_id
                    )
    ...
    public function create($data = array())
    {
           ...
           !is_null($entity_id) && !is_null($entity_name))
        {
            try {
                $em = $this->em;
                $file = new \ESERV\MAIN\MediaBundle\Entity\MediaStore();
