...
use ESERV\MAIN\Services\EntitySerializer;
...
//                                    );                    
    ...
    public function preRemove(LifecycleEventArgs $args) 
    {
        $entity = $args->getEntity();
        $entityManager = $args->getEntityManager();
        $deletedFrom = $entityManager->getClassMetadata(get_class($entity))->getName();
        $entityName = $entityManager->getClassMetadata(get_class($entity))->getTableName();
        if ($entityName == 'eserv_temp' || $entityName == 'document_queue') {
            #Do nothing as old address records should be inserted into 
            #address_history
        } else {                
            $del_rec = new \ESERV\MAIN\HistoryBundle\Entity\MtlDeletedRecord();
            $entitySerializer = new EntitySerializer($entityManager);        
            $tmp_rec = $entitySerializer->toArray($entity);   
            ...
            foreach ($tmp_rec as $key => $value) {
                ...
                array_push($results_array, array($key => $val));
            }
        
            $json = json_encode($results_array);
            $user_id = $this->container->get('security.context')->getToken()->getUser()->getId();

            $del_rec->setDeletedFrom($deletedFrom);
            $del_rec->setEntityId($tmp_rec['id']);
            $del_rec->setEntityName($entityName);
            $del_rec->setContact($entity ? (method_exists($entity, 'getContact') ? $entity->getContact() : null) : null );
            $del_rec->setColumnsAndValues($json);        
            $del_rec->setCreatedAt(new \DateTime());
            $del_rec->setCreatedBy($user_id);
            $entityManager->persist($del_rec);
            $entityManager->flush($del_rec);
        }
    }
