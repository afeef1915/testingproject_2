...
namespace WEBLOGS\MAIN\MTL\MylogsassignBundle\Services;
...
class MylogsassignBundleServices extends Controller {
    ...
    public function ListMylogsassign($options, $alias = 'p', $query_only = false) 
    {
        
//        $qb = $this->em->createQueryBuilder();
//
//        $result = $qb->select($alias)
//                        ->from('WEBLOGSMAINMTLMylogsassignBundle:Vlogsassign', $alias); //We are getting data from the 'person' table
        
        $qb = $this->em->createQueryBuilder();

//        $result = $qb->select($alias)
//                        ->from('EXAMPLEMAINCountryBundle:EservVCountry', $alias); //We are getting data from the 'person' table
   //$min=   $this->eservSecurity->getFosUserRecord(); 
       // print_r($min);die;       
 ...
 // echo $per_id;die;
   ...
   //  echo  $user_id;die; 
          $result = $qb->select('al')
                ->from('WEBLOGSMAINMTLMylogsassignBundle:Vlogsassign', 'al')
     ->where('al.mtl_person_id = :user_id')
                
 ->setParameter('user_id', $options['person_id'])
                ;
//          
//   echo     $result->getQuery()->getSQL();DIE;
        if (!$result) { 
            throw $this->createNotFoundException(
                    'No Persons found -- Function "ListMylogsassign"'  //We are throwing an exception if there are no results
            );
        } else {
            if (!$query_only) {
